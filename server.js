...
let lobby = {
  ...
  host: null,      // name of the host (only set when someone picks "Host" role)
};
...
// ============ HOST WORD & BOARD STATES ============
let hostWord = {};       // { round: "WORD" } — host-chosen word per round
let boardStates = {};    // { "PlayerName": { board, currentRow, gameOver, won, time, lastUpdate } }
const BOARD_TIMEOUT = 20000; // Remove stale board states after 20 seconds
...
function cleanStalePlayers() {
  ...
  // If the host has a separate heartbeat (hostLastSeen), check that instead of the players list
  if (lobby.host && lobby.hostLastSeen) {
    if (now - lobby.hostLastSeen > LOBBY_TIMEOUT) {
      lobby.host = null; // Host disconnected — clear host, don't reassign to a player
    }
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/lobby/join' && req.method === 'POST') {
    try {
      const body = await parseBody(req);
      const { name, round: playerRound, role } = body;
      ...
      // If joining as host, set them as host (don't add to players list)
      if (role === 'host') {
        lobby.host = name;
        // Keep host heartbeat separate — don't add to players
        lobby.hostLastSeen = Date.now();
      } else {
        lobby.players[name] = {
          joinedAt: lobby.players[name]?.joinedAt || Date.now(),
          lastSeen: Date.now(),
        };
        // Players never become host — only the "host" role can be host
      }
  ...
  if (pathname === '/api/lobby/start' && req.method === 'POST') {
    try {
      ...
      if (lobby.host && name !== lobby.host) {
        return sendJSON(res, 403, { error: 'Only the host can start the round' });
      }

      // Host must have set a word for this round
      if (!hostWord[lobby.round]) {
        return sendJSON(res, 400, { error: 'Host must set a word before starting' });
      }
  ...
  if (pathname === '/api/game/reset' && req.method === 'POST') {
    try {
      ...
      lobby.started = false;
      lobby.startTime = null;
      boardStates = {};  // Clear board states
      hostWord = {};     // Clear host words

      // Preserve the host if the reset was triggered by a host
      if (role === 'host' && name) {
        lobby.host = name;
        lobby.hostLastSeen = Date.now(); // Refresh heartbeat so host doesn't get cleaned up
      } else {
        lobby.host = null;
        lobby.hostLastSeen = null;
      }
  ...
  // Host sets the word for a round
  if (pathname === '/api/host/setword' && req.method === 'POST') {
    try {
      const body = await parseBody(req);
      const { name, round: wordRound, word } = body;
      if (!word || word.length !== 5) {
        return sendJSON(res, 400, { error: 'Word must be exactly 5 letters' });
      }
      // Only host can set the word
      if (lobby.host && name !== lobby.host) {
        return sendJSON(res, 403, { error: 'Only the host can set the word' });
      }
      hostWord[wordRound || lobby.round] = word.toUpperCase();
      return sendJSON(res, 200, { ok: true, round: wordRound || lobby.round, word: word.toUpperCase() });
    } catch (e) {
      return sendJSON(res, 400, { error: 'Invalid request' });
    }
  }

  // Players get the host-set word for their round
  if (pathname === '/api/host/getword' && req.method === 'GET') {
    const wordRound = parseInt(url.searchParams.get('round')) || lobby.round;
    const word = hostWord[wordRound];
    if (word) {
      return sendJSON(res, 200, { round: wordRound, word: word, hasWord: true });
    }
    return sendJSON(res, 200, { round: wordRound, word: null, hasWord: false });
  }

  // ---- BOARD STATE ROUTES ----

  // Player sends their board state (for host spectator view)
  if (pathname === '/api/board/update' && req.method === 'POST') {
    try {
      const body = await parseBody(req);
      const { name, board, currentRow, gameOver, won, time } = body;
  ...
  // Host gets all players' board states
  if (pathname === '/api/board/states' && req.method === 'GET') {
    // Clean stale board states
    const now = Date.now();
    for (const name of Object.keys(boardStates)) {
      if (now - boardStates[name].lastUpdate > BOARD_TIMEOUT) {
        delete boardStates[name];
      }
    }
    return sendJSON(res, 200, { states: boardStates });
  }
  ...
  if (pathname === '/api/lobby/reset' && req.method === 'POST') {
    try {
      ...
      lobby.started = false;
      lobby.startTime = null;
      boardStates = {}; // Clear board states for new round
      return sendJSON(res, 200, { ok: true, ...getLobbyStatus() });
    } catch (e) {
      return sendJSON(res, 400, { error: 'Invalid request' });
    }
