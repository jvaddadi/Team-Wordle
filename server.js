...
let lobby = {
  ...
  host: null,      // name of the host
  hasDesignatedHost: false, // true once someone joins with role 'host'
};
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/lobby/join' && req.method === 'POST') {
    try {
      ...
      if (role === 'host') {
        lobby.host = name;
        lobby.hasDesignatedHost = true; // A real host has joined
        ...
        // Only auto-assign host if no one has ever selected the host role
        if (!lobby.host && !lobby.hasDesignatedHost) {
          lobby.host = name;
        }
      }
  ...
  if (pathname === '/api/game/reset' && req.method === 'POST') {
    try {
      ...
      if (role === 'host' && name) {
        lobby.host = name;
        lobby.hasDesignatedHost = true;
      } else {
        lobby.host = null;
        lobby.hasDesignatedHost = false;
      }
